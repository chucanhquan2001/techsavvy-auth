    ...
    <div class="flex min-h-screen flex-col items-center justify-center px-4 py-12 sm:px-6">
        ...
        <div
            ...
            <div class="mb-8 text-center">
                <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Đăng nhập</h1>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="email"
                        autofocus
                        class="block w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-violet-500 focus:outline-none focus:ring-4 focus:ring-violet-500/15 @error('email') border-red-300 focus:border-red-500 focus:ring-red-500/15 @enderror"
            ...
            <p class="mt-8 text-center text-sm text-slate-600">
                ...
                <a href="{{ route('register') }}"
                    class="font-semibold text-violet-700 underline-offset-2 hover:underline">Đăng ký</a>
            </p>
        </div>
    </div>
@endsection